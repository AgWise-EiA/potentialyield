...
oni_map_c <- function(data, x,y, fill, HWAH, shp, limits){
  ...
  if (HWAH == TRUE){
    ggplot(data = data) +
      ...
      facet_wrap(~Wheather, labeller=as_labeller(c(`A`="Niño",`B`="Neutral", `C`="Niña")))+
      scale_fill_stepsn(n.breaks = 9, colours = viridis::viridis(9),name=" Yield"~(kg~ha^{-1}), limits = limits)+ theme_bw()+
      theme(legend.position = "right")+ 
      geom_sf(data=shp, fill=NA, color="white", linewidth=0.5)+
      coord_sf(expand = FALSE, xlim=c(min(data$Lon), max(data$Lon)), ylim=c(min(data$Lat), max(data$Lat)))+
...
#'     base.sub <- base.r[,c("XLAT","Lon","TRNO","TNAM","PDAT", "HDAT","CWAM","HWAH","CNAM","GNAM","NDCH","TMAXA",
...
#'   
  ...
  ## 4.4. Classify the ONI into three classes ####
...
#'   # Save the aggregated DSSAT output with ONI information
 saveRDS(dssat_oni,paste0(pathOut,"useCase_", country, "_",useCaseName,"_",Crop,"_",Extent,"_season_",Season,"_ONI.RDS" ))
  ...
  ################################################### 3.  Start here ################################################## 
  #dssat_oni <- readRDS('~/agwise-potentialyield/dataops/potentialyield/Data/useCase_Ghana_Esoko/Soybean/result/DSSAT/AOI/useCase_Ghana_Esoko_Soybean_AOI_season_1_ONI.RDS')
  dssat_oni <- readRDS(paste0(pathOut, 'useCase_', country,'_', useCaseName, '_', Crop, '_', Extent,'_season_',Season, '_ONI.RDS'))
 # n <- which(dssat_oni$Lat>0)
 # dssat_oni <- dssat_oni[-n,]
 # dssatoni <- dssat_oni[n,]
 # 
 # tete <- dssat_oni %>%
 #   filter(Loc == "Tete")
 
# Rename varieties
  unique(dssat_oni$Variety)
  dssat_oni$Variety <- gsub("900111", "short", dssat_oni$Variety)
  dssat_oni$Variety <- gsub("900112", "medium", dssat_oni$Variety)
  dssat_oni$Variety <- gsub("900113", "long", dssat_oni$Variety)
 #  
 ...
 #z <- which(dssat_oni$HWAH==0)
  
  dssat_oni <- dssat_oni[-z,]
  ## 4.5. Basic plots ####
  #if (Plot == TRUE){
    ### 4.5.1. Global plot ####
 ...
 dssat_oni.g <- dssat_oni %>%
   summarySE(measurevar="HWAH", groupvars=c("Lat","Lon", "ENSO"))
 ...
 mean.g <-oni_map(data=dssat.g.mean, x=Lon, y=Lat,shp=country_sf, fill=HWAH, HWAH= TRUE,limits=c(min.mean, max.mean))
 sd.g <-oni_map(data=dssat.g.mean, x=Lon, y=Lat,shp=country_sf, fill=sd, HWAH= FALSE, limits=c(min.sd, max.sd))
 
 mean.gc <-oni_map_c(data=dssat.g.mean, x=Lon, y=Lat,shp=country_sf, fill=HWAH, HWAH= TRUE,limits=c(min.mean, max.mean))
 mean.gc
 ...
 sd.gc <-oni_map_c(data=dssat.g.mean, x=Lon, y=Lat,shp=country_sf, fill=sd, HWAH= FALSE, limits=c(min.sd, max.sd))
 sd.gc
