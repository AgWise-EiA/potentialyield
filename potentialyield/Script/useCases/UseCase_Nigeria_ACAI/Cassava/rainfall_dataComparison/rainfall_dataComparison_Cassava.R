...
path <- "~/agwise-potentialyield/dataops/potentialyield/Data/useCase_Nigeria_ACAI/Cassava/raw/Rainfall/"
...
ggplot(summary,aes(x=NAME_2,y=totalRF, fill=source)) +
  geom_bar(stat="identity",colour="black", position=position_dodge())+ theme_bw()+
  labs(y="Total rainfall (mm)",x="Local government area")



##Plot locations 
nga2 <- gadm(country="NGA", level=1, path=tempdir())
...
ggplot() + 
  ...
  geom_text(data=nga2,aes(label = NAME_1, x = lon, y = lat), size = 2)+
  theme_bw()
