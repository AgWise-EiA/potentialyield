...
#' @param ex.name folder to write out DSSAT intermediate outputs (the soil and weather data in DSSAT format)
...
#'
#' @examples 
merge_DSSAT_ouput <- function(country, useCaseName,Crop, ex.name){
 
  # Set number of parallel workers
  cls <- parallel::makePSOCKcluster(jobs)
  doParallel::registerDoParallel(cls)
        
  setwd(paste('/home/jovyan/agwise-potentialyield/dataops/potentialyield/Data/useCase_Rwanda_DSSAT/extdata/', ex.name, sep=""))
  lf <- list.files()
  ...
  for (i in 1:length(lf)){
    ...
    saveRDS(f_all, file = "/home/jovyan/agwise-potentialyield/dataops/potentialyield/Data/useCase_Rwanda_DSSAT/Output_data/RW_DSSAT_test_maize.rds")
