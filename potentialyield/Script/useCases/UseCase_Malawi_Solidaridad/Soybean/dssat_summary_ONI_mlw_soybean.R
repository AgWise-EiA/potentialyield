...
# Rename varieties
 ...
 #Add box plot
 dssat.g.mean
 names(dssat.g.mean)[8]<-'ENSO_Phase'
 t_oni<-dssat.g.mean %>%
   ggplot(aes(x = factor(Variety), 
              y = HWAH, fill=ENSO_Phase)) +
   geom_boxplot()+
   theme_bw()+
   scale_color_manual(values=val)+ theme(axis.text.x =element_text(angle=360, hjust=1, face='bold'))+
   ylab(expression(bold('Yield'~(kg~ha^{-1})))) + xlab(expression(bold("Crop variety")))
 t_oni
