...
packages_required <- c("DSSAT","tidyverse","dplyr", "purrr", "ggridges","patchwork", "Rmisc", "terra", "cowplot","lubridate")
...
# Rename varieties
  ...
  dssat_oni$Variety <- gsub(v_levs[2], "long", dssat_oni$Variety)
  dssat_oni$Variety <- gsub(v_levs[3], "medium", dssat_oni$Variety)
 #  
  ...
  z <- which(dssat_oni$HWAH==0 & is.na(dssat_oni$ENSO) == TRUE)
    ...
    dssat_oni %>% 
      ...
      theme(legend.position = "right", 
            ...
            legend.text = element_text(size = 16),
            legend.title = element_text(size = 16, face = "bold"))+ 
      geom_pointrange(aes(ymin = HWAH - (0.5*sd), ymax = HWAH + (0.5*sd)))+
      geom_errorbar(aes(ymin = HWAH - (0.5*sd), ymax = HWAH + (0.5*sd)), width = .2, position= pd)+
 ...
 p1 <- dssat_oni %>%
   ...
   scale_fill_viridis_c(name="Yield"~(kg~ha^{-1}))+
   # scale_x_discrete(breaks = x_lab_o[seq(1, length(x_lab_o), by=5)],
 ...
 p2 <-  dssat_oni %>%
  ...
  combined_plot <- plot_grid(p1, p2, align = "h", axis = "lr", rel_heights = c(1, 1),nrow=2)
 # combined_plot <- (p1 | p2) + plot_layout(widths =c(2, 2))
 ggsave(paste0(pathOut,"useCase_", country, "_",useCaseName,"_",Crop,"_",Extent,"_season_",
               Season,"_ONI_HeatMap_Variety.pdf"), plot= combined_plot, dpi=500, width = 40, height=25, units=c("cm"))
 ...
 ggsave(paste0(pathOut,"useCase_", country, "_",useCaseName,"_",Crop,"_",Extent,"_season_",Season,"_ONI_Maps_Global_sd.pdf"), dpi=300, width = 8, height=6.89, units=c("in"))
  ...
  ###################################################################################
 ###Further classification of Seasons
 dssat_oni$slen<-dssat_oni$HDAT-dssat_oni$PDAT
 dssat_oni$pr_index<-dssat_oni$PRCP/as.numeric(dssat_oni$slen)
 
 p25 = quantile(dssat_oni$pr_index, 0.25, na.rm = TRUE)  # 33rd percentile
 p75 = quantile(dssat_oni$pr_index, 0.75, na.rm = TRUE)  # 66th percentil
 
 dssat_oni<-dssat_oni %>% mutate(dry_index = case_when( pr_index < p25 ~  "dry", pr_index >= p25 & pr_index <= p75 ~
                                               "neutral" , pr_index > p75 ~ "humid", TRUE ~ NA_character_))
 
 pd <- position_dodge(0.2)
 dssat_oni %>% 
   filter(ENSO!='NA') %>%
   summarySE(measurevar="HWAH", groupvars=c("Variety","TNAM", "dry_index"))%>% 
   ggplot(aes(x = factor(TNAM, level= x_lab_o), 
              y = HWAH, 
              group=dry_index, 
              color=dry_index)) +
   ...
   theme(legend.position = "right", 
         ...
         legend.text = element_text(size = 16),
         legend.title = element_text(size = 16, face = "bold"))+ 
   geom_pointrange(aes(ymin = HWAH - sd, ymax = HWAH + sd))+
   geom_errorbar(aes(ymin = HWAH - sd, ymax = HWAH + sd), width = .2, position= pd)+
   ylab(expression(bold('Yield'~(kg~ha^{-1})))) + xlab(expression(bold("Planting time")))
