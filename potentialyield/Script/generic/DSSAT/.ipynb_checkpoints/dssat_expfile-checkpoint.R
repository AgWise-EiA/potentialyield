...
dssat.expfile <- function(country, useCaseName, Crop, AOI = FALSE,filex_temp=TRUE, Planting_month_date=NULL,Harvest_month_date=NULL, ID="TLID",season =NULL, plantingWindow=1){  #xmin,xmax,ymin,ymax,res,jobs,ex.name,path.to.extdata){
  if(AOI == TRUE){
  ...
  results <- map(seq_along(grid[,1]), create_filex, path.to.temdata=path.to.temdata, filex_temp=filex_temp, path.to.extdata=path.to.extdata, 
                 coords=coords, AOI=AOI, code=code, plantingWindow=plantingWindow, number_years=number_years) %||% print("Progress:")
