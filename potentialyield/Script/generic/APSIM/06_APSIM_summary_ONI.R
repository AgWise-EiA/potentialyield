# Aggregated APSIM output and ONI classification of aggregated APSIM output
...
packages_required <- c("tidyverse", "ggridges","patchwork", "Rmisc", "terra", "cowplot", "foreach","doParallel","future","future.apply", "rlang")
...
#'
#' @examples 04_merge_APSIM_output(country="Rwanda", useCaseName="RAB",Crop="Maize",varietyids=c("890011","890012"), zone_folder=T, level2_folder=F)
...
get_ONI <- function(country, useCaseName, Crop, AOI=TRUE, season, Plot=TRUE,
                    short_variety, medium_variety, long_variety,
                    justplot=FALSE){
  ...
  if (justplot == FALSE) {
    ...
    ### 4.2.2. Get the aggregated APSIM output and reshape the data ####
    # Open the aggregated APSIM output
    if(AOI ==TRUE){
      apsim_path <- paste0(pathOut, "useCase_", country, "_", 
                           useCaseName, "_", Crop, "_AOI_season_", season, ".parquet")
    }else{
      apsim_path <- paste0(pathOut, "useCase_", country, "_", 
    ...
    ## 4.3. Get the mode ONI over the cropping season for aggregated APSIM output ####
    ...
    if(AOI ==TRUE){
      write_parquet(apsim_oni,paste0(pathOut,"useCase_", country, "_",useCaseName,"_",Crop,"_AOI_season_",season,"_ONI.parquet" ))
    }else{
      write_parquet(apsim_oni,paste0(pathOut,"useCase_", country, "_",useCaseName,"_",Crop,"_fieldData_season_",season,"_ONI.parquet" ))
    }
  }else{
    if(AOI ==TRUE){
      apsim_oni <- read_parquet(paste0(pathOut,"useCase_", country, "_",useCaseName,"_",Crop,"_AOI_season_",season,"_ONI.parquet" ))
    }else{
      apsim_oni <- read_parquet(paste0(pathOut,"useCase_", country, "_",useCaseName,"_",Crop,"_fieldData_season_",season,"_ONI.parquet" ))
    }
  }
